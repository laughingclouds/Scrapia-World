...
BASE_DIR = Path(__file__).resolve().parent  # BASE_DIR is the directory in which this python script lies
...
def get_chapter_number_list(child_directory_files_list: List[str]) -> List[int]:
    ...
    chapter_number_list: List[int] = [_get_number_from_string(chapter_title) for chapter_title in child_directory_files_list]
...
def check(latest_chapter: bool=False, list_all_chapters: bool=False):
    ...
    chapter_number_list: List[int] = get_chapter_number_list(child_directory_filename_list)
